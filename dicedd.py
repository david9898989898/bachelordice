...
decline_rate = 0.001  # Decline rate for the discount rate
...
# Define a new function for the declining discount rate
def declining_discount_rate(t, initial_rate, decline_rate):
    return initial_rate * np.exp(-decline_rate * t)

# Update the discount rate calculation
rr = np.zeros(NT)
for i in range(NT):
    rr[i] = 1 / ((1 + declining_discount_rate(i, prstp, decline_rate))**(tstep * i))

# Comment out the old discount rate calculation
...
def Optimality(x,iI,iK,ial,il,iYGROSS,isigma,iEIND,iE,iCCA,iCCATOT,icumetree,iMAT,iMU,iML,iFORC,iTATM,iTOCEAN,iDAMFRAC,iDAMAGES,iABATECOST,icost1,iMCABATE,
    ...
    return (resUtility[0],iI,iK,ial,il,iYGROSS,isigma,iEIND,iE,CCA,CCATOT,icumetree,iMAT,iMU,iML,iFORC,iTATM,iTOCEAN,iDAMFRAC,iDAMAGES,iABATECOST,icost1,iMCABATE,
